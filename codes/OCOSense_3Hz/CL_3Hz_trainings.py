...
for i in range(cfg.N_REPETITIONS):
    ...
    frequency, signals_to_skip = '3', 22
    ...
    print("  n. TEST OCOSense:", len(windows_test2["3"]))
    ...
    windows_test['3'] = utils.merge_user_window(windows_test2['3'], randomize=True, seed=cfg.SEEDS[i])
    ...
    labels_test['3'] = utils.merge_label(labels_test2['3'], randomize=True, seed=cfg.SEEDS[i])
    ...
    signal_sizes = [3*window_length]*22 #in this case the signal size is the same for all channels, but the model aloows different sizes
    sampling_rates = [3]*22  #in this case the sampling rate is the same for all channels, but the model aloows different sizes
    ...
    history_df.to_csv(cfg.LOGDIR + f"training_history_OCO_3Hz_CL_R{i}.csv", index=False)

    # plot 
    utils.plot_trends(train_loss, train_accuracy, train_f1, title=f'Training_OCO_3Hz_CL_R{i}', fold=cfg.IMAGES_FOLDER)
    utils.plot_trends(val_loss, val_accuracy, val_f1, title=f'Validation_OCO_3Hz_CL_R{i}', fold=cfg.IMAGES_FOLDER)
    ...
    with open(cfg.LOGDIR + f"metrics_OCO_3Hz_CL_R{i}.pkl", 'wb') as f:
...
# SAVE RESULTS
##########################################################
with open(cfg.LOGDIR + f"metrics_OCO_3Hz_CL_REP.pkl", 'wb') as f:
    pickle.dump(METRICS, f)
...
print("OCOSense")
print("Accuracy (3Hz): {:.2f} ± {:.2f}".format(np.mean(METRICS["Accuracy_3"])*100, np.std(METRICS["Accuracy_3"])*100))
print("F1 (3Hz): {:.2f} ± {:.2f}".format(np.mean(METRICS["F1_3"])*100, np.std(METRICS["F1_3"]*100)*100))
print("Loss (3Hz): {:.3f} ± {:.3f}".format(np.mean(METRICS["Loss_3"]), np.std(METRICS["Loss_3"])))
