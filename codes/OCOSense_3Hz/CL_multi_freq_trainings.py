...
for i in range(cfg.N_REPETITIONS):
    ...
    signal_sizes = [40*window_length]*22 + [3*window_length]*22 #in this case the signal size is the same for all channels, but the model aloows different sizes
    sampling_rates = [40]*22 + [3]*22 #in this case the sampling rate is the same for all channels, but the model aloows different sizes
    ...
    ##########################################################
    # reset GPU memory
    tf.keras.backend.clear_session()  #tf.config.experimental.reset_memory_stats('GPU:0')
    gc.collect()
    #device.reset()
...
print("\nAccuracy (3Hz): {:.2f} ± {:.2f}".format(np.mean(METRICS["Accuracy_3"])*100, np.std(METRICS["Accuracy_3"])*100))
print("F1 (3Hz): {:.2f} ± {:.2f}".format(np.mean(METRICS["F1_3"])*100, np.std(METRICS["F1_3"])*100))
