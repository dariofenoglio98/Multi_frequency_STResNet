...
import config_CL_down_3Hz as cfg
...
for i in range(cfg.N_REPETITIONS):
    ...
    # define checkpoint 
    model_save_path = cfg.MODELS_FOLDER + f"down_3Hz_model_CL_R{i}.h5"
    ...
    history_df.to_csv(cfg.LOGDIR + f"training_history_OCO_down3Hz_CL_R{i}.csv", index=False)

    # plot 
    utils.plot_trends(train_loss, train_accuracy, train_f1, title=f'Training_OCO_down3Hz_CL_R{i}', fold=cfg.IMAGES_FOLDER)
    utils.plot_trends(val_loss, val_accuracy, val_f1, title=f'Validation_OCO_down3Hz_CL_R{i}', fold=cfg.IMAGES_FOLDER)
    ...
    gc.collect()
    print('\n  OCOSense TESTING (Down 3Hz))')

    # Load the best model
    STResNet_model.load_weights(cfg.MODELS_FOLDER + f"down_3Hz_model_CL_R{i}.h5")
    ...
    with open(cfg.LOGDIR + f"metrics_OCO_down3Hz_CL_R{i}.pkl", 'wb') as f:
...
# SAVE RESULTS
##########################################################
with open(cfg.LOGDIR + f"metrics_OCO_CL_down3Hz_REP.pkl", 'wb') as f:
    pickle.dump(METRICS, f)
