...
for i in range(cfg.N_REPETITIONS):
    ...
    windows_train1, labels_train1, frequency_train1, windows_test1, labels_test1 = utils.split_clients_by_frequency_down(windows1, labels1, clients1, seed=cfg.SEEDS[i])

    # split train in train and val - take the first 3 clients for validation from each sensor combination
    windows_val1, labels_val1, sensors_val1 = utils.extract_validation_set(windows_train1, labels_train1, frequency_train1, 6)
    ...
    history_df.to_csv(cfg.LOGDIR + f"training_history_HEAR_down5Hz_CL_R{i}.csv", index=False)

    # plot 
    utils.plot_trends(train_loss, train_accuracy, train_f1, title=f'Training_HEAR_down5Hz_CL_R{i}', fold=cfg.IMAGES_FOLDER)
    utils.plot_trends(val_loss, val_accuracy, val_f1, title=f'Validation_HEAR_down5Hz_CL_R{i}', fold=cfg.IMAGES_FOLDER)
    ...
    with open(cfg.LOGDIR + f"metrics_HEAR_down5Hz_CL_R{i}.pkl", 'wb') as f:
...
# SAVE RESULTS
##########################################################
with open(cfg.LOGDIR + f"metrics_HEAR_CL_down5Hz_REP.pkl", 'wb') as f:
    pickle.dump(METRICS, f)
...
print("USI-HEAR (Down 5Hz)")
print("\nAccuracy (5Hz): {:.4f} ± {:.4f}".format(np.mean(METRICS["Accuracy_5"]), np.std(METRICS["Accuracy_5"])))
