...
for i in range(cfg.N_REPETITIONS):
    ...
    windows_train1, labels_train1, frequency_train1, windows_test1, labels_test1 = utils.split_clients_by_frequency(windows1, labels1, clients1, seed=cfg.SEEDS[i])

    # split train in train and val - take the first 3 clients for validation from each sensor combination
    windows_val1, labels_val1, frequency_val1 = utils.extract_validation_set(windows_train1, labels_train1, frequency_train1, 3)

    # keep only clients with 'acc' in sensors
    frequency, signals_to_skip = '5', 16
    windows_train1 = [windows_train1[i] for i in range(len(windows_train1)) if frequency in frequency_train1[i]]
    labels_train1 = [labels_train1[i] for i in range(len(labels_train1)) if frequency in frequency_train1[i]]
    windows_val1 = [windows_val1[i] for i in range(len(windows_val1)) if frequency in frequency_val1[i]]
    labels_val1 = [labels_val1[i] for i in range(len(labels_val1)) if frequency in frequency_val1[i]]

    # remove other signals from windows - use only acc
    windows_train1 = [utils.add_new_context_vect(windows_train1[i][signals_to_skip:-1]) for i in range(len(windows_train1))]
    windows_val1 = [utils.add_new_context_vect(windows_val1[i][signals_to_skip:-1]) for i in range(len(windows_val1))]
    windows_test1[frequency] = [utils.add_new_context_vect(windows_test1[frequency][i][signals_to_skip:-1]) for i in range(len(windows_test1[frequency]))]
    ...
    print("  n. TEST HEAR:", len(windows_test1["5"]))
    ...
    windows_test['5'] = utils.merge_user_window(windows_test1['5'], randomize=True, seed=cfg.SEEDS[i])
    ...
    labels_test['5'] = utils.merge_label(labels_test1['5'], randomize=True, seed=cfg.SEEDS[i])
    ...
    signal_sizes = [5*window_length]*16 #in this case the signal size is the same for all channels, but the model aloows different sizes
    sampling_rates = [5]*16  #in this case the sampling rate is the same for all channels, but the model aloows different sizes
    ...
    # define checkpoint 
    model_save_path = cfg.MODELS_FOLDER + f"5Hz_model_CL_R{i}.h5"
    ...
    history_df.to_csv(cfg.LOGDIR + f"training_history_HEAR_5Hz_CL_R{i}.csv", index=False)

    # plot 
    utils.plot_trends(train_loss, train_accuracy, train_f1, title=f'Training_HEAR_5Hz_CL_R{i}', fold=cfg.IMAGES_FOLDER)
    utils.plot_trends(val_loss, val_accuracy, val_f1, title=f'Validation_HEAR_5Hz_CL_R{i}', fold=cfg.IMAGES_FOLDER)
    ...
    gc.collect()
    print('\n  USI-HEAR TESTING (5Hz)')

    # Load the best model
    STResNet_model.load_weights(cfg.MODELS_FOLDER + f"5Hz_model_CL_R{i}.h5")

    # Evaluate the model on the test set
    test_loss5, test_accuracy5, test_f15 = cfg.test_on_one_dataset_mean(STResNet_model, windows_test['5'], labels_test['5'])

    # Print the results
    print(f"Test Loss (5Hz): {test_loss5:.4f}")
    print(f"Test Accuracy (5Hz): {test_accuracy5:.4f}")
    print(f"Test F1 (5Hz): {test_f15:.4f}")


    # save results
    METRICS["Loss_5"].append(test_loss5)
    METRICS["Accuracy_5"].append(test_accuracy5)
    METRICS["F1_5"].append(test_f15)

    with open(cfg.LOGDIR + f"metrics_HEAR_5Hz_CL_R{i}.pkl", 'wb') as f:
...
# SAVE RESULTS
##########################################################
with open(cfg.LOGDIR + f"metrics_HEAR_5Hz_CL_REP.pkl", 'wb') as f:
    pickle.dump(METRICS, f)
...
print("USI-HEAR")
print("Accuracy (5Hz): {:.4f} ± {:.4f}".format(np.mean(METRICS["Accuracy_5"]), np.std(METRICS["Accuracy_5"])))
print("F1 (5Hz): {:.4f} ± {:.4f}".format(np.mean(METRICS["F1_5"]), np.std(METRICS["F1_5"])))
print("Loss (5Hz): {:.4f} ± {:.4f}".format(np.mean(METRICS["Loss_5"]), np.std(METRICS["Loss_5"])))
