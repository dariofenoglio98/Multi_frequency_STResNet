...
for i in range(cfg.N_REPETITIONS):
    ...
    frequency, signals_to_keep = '40', 16
    ...
    windows_train1 = [utils.add_new_context_vect(windows_train1[i][:signals_to_keep]) for i in range(len(windows_train1))]
    windows_val1 = [utils.add_new_context_vect(windows_val1[i][:signals_to_keep]) for i in range(len(windows_val1))]
    windows_test1[frequency] = [utils.add_new_context_vect(windows_test1[frequency][i][:signals_to_keep]) for i in range(len(windows_test1[frequency]))]
    ...
    signal_sizes = [40*window_length]*16 #in this case the signal size is the same for all channels, but the model aloows different sizes
    sampling_rates = [40]*16  #in this case the sampling rate is the same for all channels, but the model aloows different sizes
    ...
    # define checkpoint 
    model_save_path = cfg.MODELS_FOLDER + f"40Hz_model_CL_R{i}.h5"
    ...
    history_df.to_csv(cfg.LOGDIR + f"training_history_HEAR_40Hz_CL_R{i}.csv", index=False)

    # plot 
    utils.plot_trends(train_loss, train_accuracy, train_f1, title=f'Training_HEAR_40Hz_CL_R{i}', fold=cfg.IMAGES_FOLDER)
    utils.plot_trends(val_loss, val_accuracy, val_f1, title=f'Validation_HEAR_40Hz_CL_R{i}', fold=cfg.IMAGES_FOLDER)
    ...
    gc.collect()
    print('\n  USI-HEAR TESTING (40Hz)')

    # Load the best model
    STResNet_model.load_weights(cfg.MODELS_FOLDER + f"40Hz_model_CL_R{i}.h5")
    ...
    with open(cfg.LOGDIR + f"metrics_HEAR_40Hz_CL_R{i}.pkl", 'wb') as f:
...
# SAVE RESULTS
##########################################################
with open(cfg.LOGDIR + f"metrics_HEAR_40Hz_CL_REP.pkl", 'wb') as f:
    pickle.dump(METRICS, f)
