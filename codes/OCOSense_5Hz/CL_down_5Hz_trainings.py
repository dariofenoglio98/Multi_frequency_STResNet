...
import config_CL_down_5Hz as cfg
...
for i in range(cfg.N_REPETITIONS):
    ...
    windows_train2, labels_train2, frequency_train2, windows_test2, labels_test2 = utils.split_clients_by_frequency_down(windows2, labels2, clients2, client_split=16, seed=cfg.SEEDS[i])

    # split train in train and val - take the first 3 clients for validation from each sensor combination
    windows_val2, labels_val2, sensors_val2 = utils.extract_validation_set(windows_train2, labels_train2, frequency_train2, 6)
    ...
    signal_sizes = [5*window_length]*22 #in this case the signal size is the same for all channels, but the model allows different sizes
    sampling_rates = [5]*22 #in this case the sampling rate is the same for all channels, but the model allows different sizes
    ...
    # define checkpoint 
    model_save_path = cfg.MODELS_FOLDER + f"down_5Hz_model_CL_R{i}.h5"
    ...
    history_df.to_csv(cfg.LOGDIR + f"training_history_OCO_down5Hz_CL_R{i}.csv", index=False)

    # plot 
    utils.plot_trends(train_loss, train_accuracy, train_f1, title=f'Training_OCO_down5Hz_CL_R{i}', fold=cfg.IMAGES_FOLDER)
    utils.plot_trends(val_loss, val_accuracy, val_f1, title=f'Validation_OCO_down5Hz_CL_R{i}', fold=cfg.IMAGES_FOLDER)
    ...
    gc.collect()
    print('\n  OCOSense TESTING (Down 5Hz))')

    # Load the best model
    STResNet_model.load_weights(cfg.MODELS_FOLDER + f"down_5Hz_model_CL_R{i}.h5")

    # Evaluate the model on the test set
    test_loss5, test_accuracy5, test_f15 = cfg.test_on_one_dataset_mean(STResNet_model, windows_test['5'], labels_test['5'])

    # Print the results
    print(f"\nTest Loss (5Hz): {test_loss5:.4f}")
    ...
    with open(cfg.LOGDIR + f"metrics_OCO_down5Hz_CL_R{i}.pkl", 'wb') as f:
...
# SAVE RESULTS
##########################################################
with open(cfg.LOGDIR + f"metrics_OCO_CL_down5Hz_REP.pkl", 'wb') as f:
    pickle.dump(METRICS, f)
...
print("OCOSense (Down 5Hz)")
print("\nAccuracy (5Hz): {:.2f} ± {:.2f}".format(np.mean(METRICS["Accuracy_5"])*100, np.std(METRICS["Accuracy_5"])*100))
print("F1 (5Hz): {:.2f} ± {:.2f}".format(np.mean(METRICS["F1_5"])*100, np.std(METRICS["F1_5"])*100))
