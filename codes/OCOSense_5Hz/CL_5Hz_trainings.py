...
for i in range(cfg.N_REPETITIONS):
    ...
    windows_train2, labels_train2, frequency_train2, windows_test2, labels_test2 = utils.split_clients_by_frequency_40_down(windows2, labels2, clients2, client_split=[8,16], seed=cfg.SEEDS[i])

    # split train in train and val - take the first 3 clients for validation from each sensor combination
    windows_val2, labels_val2, frequency_val2 = utils.extract_validation_set(windows_train2, labels_train2, frequency_train2, 3)

    # keep only clients with 'acc' in sensors
    frequency, signals_to_skip = '5', 22
    ...
    windows_train2 = [utils.add_new_context_vect(windows_train2[i][signals_to_skip:-1]) for i in range(len(windows_train2))]
    windows_val2 = [utils.add_new_context_vect(windows_val2[i][signals_to_skip:-1]) for i in range(len(windows_val2))]
    windows_test2[frequency] = [utils.add_new_context_vect(windows_test2[frequency][i][signals_to_skip:-1]) for i in range(len(windows_test2[frequency]))]
    ...
    print("  n. TEST OCOSense:", len(windows_test2["5"]))
    ...
    windows_test['5'] = utils.merge_user_window(windows_test2['5'], randomize=True, seed=cfg.SEEDS[i])
    ...
    labels_test['5'] = utils.merge_label(labels_test2['5'], randomize=True, seed=cfg.SEEDS[i])
    ...
    signal_sizes = [5*window_length]*22 #in this case the signal size is the same for all channels, but the model aloows different sizes
    sampling_rates = [5]*22  #in this case the sampling rate is the same for all channels, but the model aloows different sizes
    ...
    history_df.to_csv(cfg.LOGDIR + f"training_history_OCO_5Hz_CL_R{i}.csv", index=False)

    # plot 
    utils.plot_trends(train_loss, train_accuracy, train_f1, title=f'Training_OCO_5Hz_CL_R{i}', fold=cfg.IMAGES_FOLDER)
    utils.plot_trends(val_loss, val_accuracy, val_f1, title=f'Validation_OCO_5Hz_CL_R{i}', fold=cfg.IMAGES_FOLDER)
    ...
    with open(cfg.LOGDIR + f"metrics_OCO_5Hz_CL_R{i}.pkl", 'wb') as f:
...
# SAVE RESULTS
##########################################################
with open(cfg.LOGDIR + f"metrics_OCO_5Hz_CL_REP.pkl", 'wb') as f:
    pickle.dump(METRICS, f)
...
print("OCOSense")
print("Accuracy (5Hz): {:.2f} ± {:.2f}".format(np.mean(METRICS["Accuracy_5"])*100, np.std(METRICS["Accuracy_5"])*100))
print("F1 (5Hz): {:.2f} ± {:.2f}".format(np.mean(METRICS["F1_5"])*100, np.std(METRICS["F1_5"]*100)*100))
print("Loss (5Hz): {:.3f} ± {:.3f}".format(np.mean(METRICS["Loss_5"]), np.std(METRICS["Loss_5"])))
