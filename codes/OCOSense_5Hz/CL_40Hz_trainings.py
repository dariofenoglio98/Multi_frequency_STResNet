...
for i in range(cfg.N_REPETITIONS):
    ...
    frequency, signals_to_keep = '40', 22 #first 22 signals are 40Hz
    ...
    history_df.to_csv(cfg.LOGDIR + f"training_history_OCO_40Hz_CL_R{i}.csv", index=False)

    # plot 
    utils.plot_trends(train_loss, train_accuracy, train_f1, title=f'Training_OCO_40Hz_CL_R{i}', fold=cfg.IMAGES_FOLDER)
    utils.plot_trends(val_loss, val_accuracy, val_f1, title=f'Validation_OCO_40Hz_CL_R{i}', fold=cfg.IMAGES_FOLDER)
    ...
    with open(cfg.LOGDIR + f"metrics_OCO_40Hz_CL_R{i}.pkl", 'wb') as f:
...
# SAVE RESULTS
##########################################################
with open(cfg.LOGDIR + f"metrics_OCO_40Hz_CL_REP.pkl", 'wb') as f:
    pickle.dump(METRICS, f)
