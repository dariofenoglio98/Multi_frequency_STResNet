...
# Split clients by frequency (40Hz and 5Hz)
def split_channels_by_frequency(features, frequency=None):
    ...
    if frequency == "40":
        ...
        missing_data = np.zeros((features.shape[0], int(features.shape[1]/8), n_channels))
        ...
        features = resample(features, num=int(features.shape[1]/8), axis=1)
        features = np.concatenate([features],axis=2)
        features = np.split(features, features.shape[2], axis=2)
...
def split_clients_by_frequency(windows, labels, clients, seed=42):
    ...
    labels_clients = []
    frequency_clients = []
    windows_clients_test = {"40":[], "5":[]}
    labels_clients_test = {"40":[], "5":[]}
    ...
    f40, f5 = random_clients[:10], random_clients[10:20], 
    ...
    for client in np.unique(clients):
        ...
        # if clients have accelerometer and gyroscope
            ...
            np.random.shuffle(client_idx)
            windows_client = windows[client_idx]
            windows_client = split_channels_by_frequency(windows_client, frequency="5")
            # append the windows to the list
            windows_clients.append(windows_client)
            labels_clients.append(labels[client_idx])
            frequency_clients.append("5")
        # if clients are in the test
            ...
            w = split_channels_by_frequency(windows_client, frequency="5")
            windows_clients_test["5"].append(w)
            labels_clients_test["5"].append(labels[client_idx])
...
def split_clients_by_frequency_down(windows, labels, clients, client_split=20, seed=42):
    # split the windows by client, create a list
    windows_clients = []
    labels_clients = []
    frequency_clients = []
    windows_clients_test = {"5":[]}
    labels_clients_test = { "5":[]}
    # randomly shuffle clients
    random_clients = np.unique(clients)
    # fix the seed
    np.random.seed(seed)
    ...
    # divide in 4 groups of 5 clients
    f5 = random_clients[:client_split] 
    test_user = random_clients[client_split:]
    print("  Clients in the test set: ", test_user)
    ...
    for client in np.unique(clients):
        ...
        if client in f5:
            ...
            np.random.shuffle(client_idx)
            windows_client = windows[client_idx]
            windows_client = split_channels_by_frequency_down(windows_client, frequency="5")
            # append the windows to the list
            windows_clients.append(windows_client)
            labels_clients.append(labels[client_idx])
            frequency_clients.append("5")
        # if clients are in the test
            ...
            windows_client = windows[client_idx]
            # accelerometer and gyroscope
            w = split_channels_by_frequency_down(windows_client, frequency="5")
            windows_clients_test["5"].append(w)
            labels_clients_test["5"].append(labels[client_idx])
...
def split_channels_by_frequency_40down(features, frequency=None):
    ...
    if frequency == "40":
        ...
        context = np.tile(context, (features.shape[0], 1))
        features_down = resample(features, num=int(features.shape[1]/8), axis=1)
...
def split_clients_by_frequency_40_down(windows, labels, clients, client_split=[10,20], seed=42):
    # split the windows by client, create a list
    windows_clients = []
    labels_clients = []
    frequency_clients = []
    ...
    # divide in 4 groups of 5 clients
    f40, f5 = random_clients[:client_split[0]], random_clients[client_split[0]:client_split[1]], 
    test_user = random_clients[client_split[1]:]
    print("  Clients in the test set: ", test_user)
    ...
    for client in np.unique(clients):
        ...
        # if clients have accelerometer and gyroscope
            ...
            np.random.shuffle(client_idx)
            windows_client = windows[client_idx]
            windows_client = split_channels_by_frequency_40down(windows_client, frequency="5")
            # append the windows to the list
            windows_clients.append(windows_client)
            labels_clients.append(labels[client_idx])
            frequency_clients.append("5")
        # if clients are in the test
            ...
            labels_clients_test["40"].append(labels[client_idx])
            # accelerometer and gyroscope
            w = split_channels_by_frequency_40down(windows_client, frequency="5")
            windows_clients_test["5"].append(w)
            labels_clients_test["5"].append(labels[client_idx])
...
# Create all clients with 40Hz
def split_clients_by_frequency_40_all(windows, labels, clients, client_split=20, seed=42):
    # split the windows by client, create a list
    windows_clients = []
    labels_clients = []
    frequency_clients = []
    ...
    # divide in 4 groups of 5 clients
    f40 = random_clients[:client_split] 
    test_user = random_clients[client_split:]
    print("  Clients in the test set: ", test_user)
