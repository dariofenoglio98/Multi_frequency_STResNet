...
for i in range(cfg.N_REPETITIONS):
    ...
    windows_val2, labels_val2, sensors_val2 = utils.extract_validation_set(windows_train2, labels_train2, frequency_train2, 3)
    ...
    signal_sizes = [40*window_length]*22 + [5*window_length]*22 # different signal sizes for each channel
    sampling_rates = [40]*22 + [5]*22 # different sampling rates for each channel
    ...
    history_df.to_csv(cfg.LOGDIR + f"training_history_OCO_CL_R{i}.csv", index=False)

    # plot 
    utils.plot_trends(train_loss, train_accuracy, train_f1, title=f'Training_OCO_CL_R{i}', fold=cfg.IMAGES_FOLDER)
    utils.plot_trends(val_loss, val_accuracy, val_f1, title=f'Validation_OCO_CL_R{i}', fold=cfg.IMAGES_FOLDER)
    ...
    with open(cfg.LOGDIR + f"metrics_OCO_CL_R{i}.pkl", 'wb') as f:
...
# SAVE RESULTS
##########################################################
with open(cfg.LOGDIR + f"metrics_OCO_CL_REP.pkl", 'wb') as f:
    pickle.dump(METRICS, f)
...
print("OCOSense")
print("Accuracy (40Hz): {:.2f} ± {:.2f}".format(np.mean(METRICS["Accuracy_40"])*100, np.std(METRICS["Accuracy_40"])*100))
print("F1 (40Hz): {:.2f} ± {:.2f}".format(np.mean(METRICS["F1_40"])*100, np.std(METRICS["F1_40"])*100))
print("Loss (40Hz): {:.3f} ± {:.3f}".format(np.mean(METRICS["Loss_40"]), np.std(METRICS["Loss_40"])))
