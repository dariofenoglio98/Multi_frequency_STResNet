...
for i in range(cfg.N_REPETITIONS):
    ...
    windows_train1, labels_train1, frequency_train1, windows_test1, labels_test1 = utils.split_clients_by_frequency_40_all(windows1, labels1, clients1, seed=cfg.SEEDS[i])

    # split train in train and val - take the first 6 clients for validation from each sensor combination
    windows_val1, labels_val1, frequency_val1 = utils.extract_validation_set(windows_train1, labels_train1, frequency_train1, 6)
    ...
    # Parameters
    print('  USI-HEAR TRAINING (40Hz_all)')
    ...
    history_df.to_csv(cfg.LOGDIR + f"training_history_HEAR_40Hz_all_CL_R{i}.csv", index=False)

    # plot 
    utils.plot_trends(train_loss, train_accuracy, train_f1, title=f'Training_HEAR_40Hz_all_CL_R{i}', fold=cfg.IMAGES_FOLDER)
    utils.plot_trends(val_loss, val_accuracy, val_f1, title=f'Validation_HEAR_40Hz_all_CL_R{i}', fold=cfg.IMAGES_FOLDER)
    ...
    with open(cfg.LOGDIR + f"metrics_HEAR_40Hz_all_CL_R{i}.pkl", 'wb') as f:
...
# SAVE RESULTS
##########################################################
with open(cfg.LOGDIR + f"metrics_HEAR_40Hz_all_CL_REP.pkl", 'wb') as f:
    pickle.dump(METRICS, f)
...
print("USI-HEAR")
print("Accuracy (40Hz_all): {:.4f} ± {:.4f}".format(np.mean(METRICS["Accuracy_40"]), np.std(METRICS["Accuracy_40"])))
print("F1 (40Hz_all): {:.4f} ± {:.4f}".format(np.mean(METRICS["F1_40"]), np.std(METRICS["F1_40"])))
print("Loss (40Hz_all): {:.4f} ± {:.4f}".format(np.mean(METRICS["Loss_40"]), np.std(METRICS["Loss_40"])))
