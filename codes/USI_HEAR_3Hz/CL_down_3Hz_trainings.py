...
for i in range(cfg.N_REPETITIONS):
    ...
    history_df.to_csv(cfg.LOGDIR + f"training_history_HEAR_down3Hz_CL_R{i}.csv", index=False)

    # plot 
    utils.plot_trends(train_loss, train_accuracy, train_f1, title=f'Training_HEAR_down3Hz_CL_R{i}', fold=cfg.IMAGES_FOLDER)
    utils.plot_trends(val_loss, val_accuracy, val_f1, title=f'Validation_HEAR_down3Hz_CL_R{i}', fold=cfg.IMAGES_FOLDER)
    ...
    with open(cfg.LOGDIR + f"metrics_HEAR_down3Hz_CL_R{i}.pkl", 'wb') as f:
...
# SAVE RESULTS
##########################################################
with open(cfg.LOGDIR + f"metrics_HEAR_CL_down3Hz_REP.pkl", 'wb') as f:
    pickle.dump(METRICS, f)
