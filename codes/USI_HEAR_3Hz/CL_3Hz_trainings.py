...
for i in range(cfg.N_REPETITIONS):
    ...
    frequency, signals_to_skip = '3', 16
    ...
    print("  n. TEST HEAR:", len(windows_test1["3"]))
    ...
    signal_sizes = [3*window_length]*16 #in this case the signal size is the same for all channels, but the model aloows different sizes
    sampling_rates = [3]*16  #in this case the sampling rate is the same for all channels, but the model aloows different sizes
    ...
    # define checkpoint 
    model_save_path = cfg.MODELS_FOLDER + f"3Hz_model_CL_R{i}.h5"
    ...
    history_df.to_csv(cfg.LOGDIR + f"training_history_HEAR_3Hz_CL_R{i}.csv", index=False)

    # plot 
    utils.plot_trends(train_loss, train_accuracy, train_f1, title=f'Training_HEAR_3Hz_CL_R{i}', fold=cfg.IMAGES_FOLDER)
    utils.plot_trends(val_loss, val_accuracy, val_f1, title=f'Validation_HEAR_3Hz_CL_R{i}', fold=cfg.IMAGES_FOLDER)
    ...
    gc.collect()
    print('\n  USI-HEAR TESTING (3Hz)')

    # Load the best model
    STResNet_model.load_weights(cfg.MODELS_FOLDER + f"3Hz_model_CL_R{i}.h5")

    # Evaluate the model on the test set
    test_loss3, test_accuracy3, test_f13 = cfg.test_on_one_dataset_mean(STResNet_model, windows_test['3'], labels_test['3'])

    # Print the results
    print(f"Test Loss (3Hz): {test_loss3:.4f}")
    ...
    with open(cfg.LOGDIR + f"metrics_HEAR_3Hz_CL_R{i}.pkl", 'wb') as f:
...
# SAVE RESULTS
##########################################################
with open(cfg.LOGDIR + f"metrics_HEAR_3Hz_CL_REP.pkl", 'wb') as f:
    pickle.dump(METRICS, f)
...
print("USI-HEAR")
print("Accuracy (3Hz): {:.4f} ± {:.4f}".format(np.mean(METRICS["Accuracy_3"]), np.std(METRICS["Accuracy_3"])))
